...
<body>
   <main>
      ...
      <div class="container">
         ...
         <table id="table">
            ...
            <tr>
               <td>글번호</td>
               <td style = "color:black;">${ vo.ccomu_bno }</td>
               <td>조회수</td>
               <td style = "color:black;">${ vo.ccomu_hit }</td>
            </tr>
            <tr>
               <td>작성자</td>
               <td style = "color:black;">${ vo.com_id }</td>
               <td>작성일</td>
               <td style = "color:black;">${ vo.ccomu_date }</td>
            </tr>
            <tr>
               <td>제목</td>
               <td colspan="3" style = "color:black;">${ vo.ccomu_title }</td>
            </tr>
            <tr>
               <td>내용</td>
               <td colspan="3" style = "color:black;">${ vo.ccomu_content }</td>
            </tr>
      ...
      <div class="container2" style="width: 95%;">
        <c:forEach items="${commentList}" var="comment">
          <div class="card m-2" id="comments-${comment.ccomu_bno}">
            <div class="card-header">${comment.com_id}&nbsp;&nbsp;&nbsp;</div>
              <div class="card-body" style="display: flex;">
                <div style="font-size: 20px;">${comment.ccomm_body}</div>
                  <div class="comment thumb-icon" id="thumb-icon-${comment.ccomm_id}" data-comment-id="${comment.ccomm_id}"></div>
                  <div><span style="font-size: 20px;">${comment.ccomu_good}</span></div>
            </div>
          </div>
        </c:forEach>
      </div>
  ...
  function loadThumbInfo(ccomm_id, ccomu_bno, com_id) {
    $.ajax({
      type: 'GET',
      url: `http://localhost:9090/Api/Comment/\${ccomu_bno}/\${ccomm_id}/commentLike`,
      data: {
        ccomm_id: ccomm_id,
        ccomu_bno: ccomu_bno,
        com_id: com_id
      ...
      success: function(data) {
        const thumbEl = document.querySelector(`#thumb-icon-\${ccomm_id}`);
        if (data == 1) {
  ...
  document.querySelectorAll('.thumb-icon').forEach(thumbIcon => {
    const ccomm_id = thumbIcon.getAttribute('data-comment-id');
